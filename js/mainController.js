  ...
  .controller('MainCtrl', ['$scope', '$filter', '$window', 'Utils', 'Auth',
    ...
    function ($scope, $filter, $window, utils, auth, $rootScope, firebaseService, modalService) {
      ...
      $scope.createNewBoard = function () {
        ...
        const callback = function (userData) {
          const board = firebaseService.getBoardRef($scope.userId);
          board.set({
      ...
      $scope.updateSortOrder = function () {
        const updatedFilter = `${$window.location.origin + $window.location.pathname}?sort=${$scope.sortField}${$window.location.hash}`;
        $window.history.pushState({ path: updatedFilter }, '', updatedFilter);
      };

      $scope.addNewColumn = function (name) {
        if (angular.isUndefined(typeof name) || name === '') {
          return;
        }
      ...
      $scope.changeColumnName = function (id, newName) {
        if (angular.isUndefined(typeof newName) || newName === '') {
          return;
        }

        $scope.board.columns.map((column, index, array) => {
          if (column.id === id) {
      ...
      $scope.deleteColumn = function (column) {
        $scope.board.columns = $scope.board.columns.filter(_column => _column.id !== column.id);
