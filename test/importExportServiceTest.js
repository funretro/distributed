describe('ImportExportService: ', () => {
  ...
  describe('Export', () => {
    ...
    it('should get board text', () => {
      const boardText = importExportService.getBoardText(board, messages, 'votes');
    ...
    it('should return empty board text', () => {
      const boardText = importExportService.getBoardText(undefined, messages, 'votes');

      expect(boardText).to.equal('');
    });

    it('should get pure board text', () => {
      const boardText = importExportService.getBoardPureText(board, messages, 'votes');

      expect(boardText).to.equal('columnName\n- C3R1 (0 votes) \n\notherColumnName\n- C3R2 (0 votes) \n- C3R3 (0 votes) \n');
    });

    it('should return empty board text', () => {
      const boardText = importExportService.getBoardPureText(undefined, messages, 'votes');
  ...
  describe('Import', () => {
    ...
    beforeEach(() => {
      sinon.stub(firebaseService, 'getBoardColumns', () => boardColumns);
    });
    ...
    it('should import mapped data', () => {
      const messageDate = Date.parse('Mon Apr 03 2017 21:07:31 GMT+0200 (W. Europe Daylight Time)');
